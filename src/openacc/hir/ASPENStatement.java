 ...
 *
 */
public abstract class ASPENStatement implements Cloneable, Traversable, ASPENPrintable {
	 ...
	 * 
	 */
	protected ASPENStatement() {
		parent = null;
		children = new ArrayList<Traversable>(1);
	}

	protected ASPENStatement(int size) {
		parent = null;
        if (size < 0) {
            children = empty_list;
        } else {
    ...
    @Override
	public ASPENStatement clone() {
        ASPENStatement o = null;
        try {
            o = (ASPENStatement)super.clone();
        ...
        if (children != null) {
            ...
            for (int i = 0; i < children_size; i++) {
                ...
                if (child instanceof ASPENStatement) {
                    o_child = ((ASPENStatement)child).clone();
                } else if (child instanceof ASPENExpression) {
                    o_child = ((ASPENExpression)child).clone();
                } else if (child instanceof Expression) {
                    o_child = ((Expression)child).clone();
                } else if (child != null) {
                    throw new InternalError(
                            "ASPENStatement contains an unknown child type" + this);
                }
                if (o_child != null) {
                    o_child.setParent(o);
                }
    ...
    */
    public void swapWith(ASPENStatement stmt) {
        if (stmt == null) {
            throw new IllegalArgumentException();
        }
        if (this == stmt) {
