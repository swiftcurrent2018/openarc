	    				...
	    				if(d instanceof VariableDeclaration && params.size() == 1){
		    				...
		    				if(decl instanceof NestedDeclarator){
		    					...
		    					if(declarator instanceof VariableDeclarator) {
			    					//[DEBUG] Added by Seyong Lee: Initialization of constant variable/pointer should not be separated.
			    					List<Specifier> declrSpecs = declarator.getSpecifiers();
			    					List<Specifier> declSpecs = p.getSpecifiers();
			    					//[FIXME] Added by Seyong Lee: Below will cause another problem if its initial value defined later than this 
			    					//constant variable. To fix this problem temporarily, constant is removed from its parameter.
/*			    					if( declrSpecs.contains(PointerSpecifier.CONST) || declrSpecs.contains(PointerSpecifier.CONST_RESTRICT) || 
			    							...
			    							(declSpecs.contains(Specifier.STATIC)) ) {
			    						Expression arg = (Expression)fc.getArgument(k).clone(); // don't forget to clone
			    						newND.setInitializer(new Initializer(arg));
			    						initialized = true;
			    					}*/
			    					if( declrSpecs.contains(PointerSpecifier.CONST) ) {
			    						declrSpecs.remove(PointerSpecifier.CONST);
			    						declrSpecs.add(PointerSpecifier.UNQUALIFIED);
			    					} else if( declrSpecs.contains(PointerSpecifier.CONST_RESTRICT) ) {
			    						declrSpecs.remove(PointerSpecifier.CONST_RESTRICT);
			    						declrSpecs.add(PointerSpecifier.UNQUALIFIED);
			    						declrSpecs.add(Specifier.RESTRICT);
			    					} else if( declrSpecs.contains(PointerSpecifier.CONST_RESTRICT_VOLATILE) ) {
			    						declrSpecs.remove(PointerSpecifier.CONST_RESTRICT_VOLATILE);
			    						declrSpecs.add(PointerSpecifier.UNQUALIFIED);
			    						declrSpecs.add(Specifier.RESTRICT);
			    						declrSpecs.add(Specifier.VOLATILE);
			    					} else if( declrSpecs.contains(PointerSpecifier.CONST_VOLATILE) ) {
			    						declrSpecs.remove(PointerSpecifier.CONST_VOLATILE);
			    						declrSpecs.add(PointerSpecifier.UNQUALIFIED);
			    						declrSpecs.add(Specifier.VOLATILE);
			    					}
			    					...
			    					VariableDeclarator vd = new VariableDeclarator(declrSpecs, nameId, originalVD.getTrailingSpecifiers());
	...
	//								}
			    				...
			    				if(declarator.getArraySpecifiers().size() > 0){
			    					...
			    					//[DEBUG] Added by Seyong Lee: Initialization of constant variable/pointer should not be separated.
			    					List<Specifier> declrSpecs = declarator.getSpecifiers();
			    					List<Specifier> declSpecs = declaration.getSpecifiers();
			    					//[FIXME] Added by Seyong Lee: Below will cause another problem if its initial value defined later than this 
			    					//constant variable. To fix this problem temporarily, constant is removed from its parameter.
/*			    					if( declrSpecs.contains(PointerSpecifier.CONST) || declrSpecs.contains(PointerSpecifier.CONST_RESTRICT) || 
			    							declrSpecs.contains(PointerSpecifier.CONST_RESTRICT_VOLATILE) || declrSpecs.contains(PointerSpecifier.CONST_VOLATILE) ||
			    							(declSpecs.contains(Specifier.CONST)&&!SymbolTools.isPointer(vd)) || (declSpecs.contains(Specifier.STATIC)) ) {
			    						Expression arg = (Expression)fc.getArgument(k).clone(); // don't forget to clone
			    						vd.setInitializer(new Initializer(arg));
			    						initialized = true;
			    					}*/
			    					if( declrSpecs.contains(PointerSpecifier.CONST) ) {
			    						declrSpecs.remove(PointerSpecifier.CONST);
			    					...
			    					if( declSpecs.contains(Specifier.CONST) && !SymbolTools.isPointer(declarator) ) {
			    						declSpecs.remove(Specifier.CONST);
			    					}
			    					...
			    					VariableDeclarator vd = new VariableDeclarator(declrSpecs, nameId);
			    					id = new Identifier(vd);
			    					//newDeclaration = new VariableDeclaration(vd);	    					
			    					//newDeclaration = new VariableDeclaration(declaration.getSpecifiers(), vd);
			    					newDeclaration = new VariableDeclaration(declSpecs, vd);
			    				}
		    				}
		    				else{
				...
				if(inline){
					...
					if(returnStmt != null){
						if(returnStmt.getExpression() != null){
							...
							//[DEBUG] Added by Seyong Lee
							//inline keyword should be removed from the variable type too.
							returnType.remove(Specifier.INLINE);
							...
							for(Annotation a : annotations){
								...
								while(iter.hasNext()){
									...
									if( key.equals("pragma") ) {
										//All non-builtin pragma annotations (e.g., OpenACC pragmas) will exist as 
										//generic pragma annotations when this pass is called.
										//OpenACC directives should be replaced here.
										if( val instanceof String ) {
											String old_annot = (String)val;
											StringBuilder str = new StringBuilder(old_annot.length());
											old_annot = ACCAnnotationParser.modifyAnnotationString(old_annot);
											String[] token_array = old_annot.split("\\s+");
											boolean modified = false;
											boolean nonFirstToken = false;
											for(String token : token_array) {
												if( nonFirstToken ) {
													str.append(" ");
												} else {
													nonFirstToken = true;
												}
												if( token.equals(oldVarName) ) {
													str.append(newVarNames.get(i));
													modified = true;
												} else {
													str.append(token);
												}
