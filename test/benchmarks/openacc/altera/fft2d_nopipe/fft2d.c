...
#pragma acc declare create(chan0[0:N*N/8], chan1[0:N*N/8], chan2[0:N*N/8], chan3[0:N*N/8], chan4[0:N*N/8], chan5[0:N*N/8], chan6[0:N*N/8], chan7[0:N*N/8], chanin0[0:N*N/8], chanin1[0:N*N/8], chanin2[0:N*N/8], chanin3[0:N*N/8], chanin4[0:N*N/8], chanin5[0:N*N/8], chanin6[0:N*N/8], chanin7[0:N*N/8])
...
static void fetch(flt2 * src, int mangle) {
	int gid, wid;
#pragma acc kernels loop gang(N/8) present(src[0:N*N]) present(chanin0, chanin1, chanin2, chanin3, chanin4, chanin5, chanin6, chanin7)
...
static void fft2d(int inverse) {
	...
	*/  
	...
	#pragma acc parallel num_gangs(1) num_workers(1) present(chanin0, chanin1, chanin2, chanin3, chanin4, chanin5, chanin6, chanin7) present(chan0, chan1, chan2, chan3, chan4, chan5, chan6, chan7)
#else
	#pragma acc parallel num_gangs(1) num_workers(1) present(chanin0, chanin1, chanin2, chanin3, chanin4, chanin5, chanin6, chanin7) present(chan0, chan1, chan2, chan3, chan4, chan5, chan6, chan7) create(fft_delay_elements[0: N + POINTS * (LOGN -2)])
#endif
	{
	int i;
#if DELAY_ELEMENTS_ON_LOCAL == 1
...
static void transpose(flt2 *dest, int mangle) {
	int gid, wid;
#pragma acc kernels loop gang(N/8) present(dest) present(chan0, chan1, chan2, chan3, chan4, chan5, chan6, chan7)
	for(gid=0; gid<N/8; gid++) {
		flt2 buf[POINTS * N]; 
#pragma acc loop worker(N)
		for(wid=0; wid<N; wid++) {
