...
static void fft2d(int inverse) {
	#pragma acc kernels loop gang worker pipein(chanin0, chanin1, chanin2, chanin3, chanin4, chanin5, chanin6, chanin7) pipeout(chan0, chan1, chan2, chan3, chan4, chan5, chan6, chan7)
	for (int m = 0; m < N; m++) {
	#pragma acc loop seq
	for (int i = 0; i < (N / POINTS) + N / POINTS - 1; i++) {
