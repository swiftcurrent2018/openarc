    ...
    float dW[_SIZE_I_], float dE[_SIZE_I_], float c[_SIZE_I_])
{
  ...
  #pragma acc data copyin(q0sqr) present(J, dN, dS, dE, dW, c) pipe(c[0:_ROWS_*_COLS_])
  {
    #pragma acc parallel loop num_gangs(1) num_workers(1) collapse(2) pipeout(c)
    ...
    #pragma acc parallel loop num_gangs(1) num_workers(1) collapse(2) pipein(c) 
