...
int n_in, n_hidden, n_out;
{
  ...
  input_units = (float *)acc_create_unified(NULL, sizeof(float)*(n_in+1)); 
  hidden_units = (float *)acc_create_unified(NULL, sizeof(float)*(n_hidden+1)); 
  output_units = alloc_1d_dbl(n_out + 1);

  //hidden_delta = alloc_1d_dbl(n_hidden + 1);
  //output_delta = alloc_1d_dbl(n_out + 1);
  hidden_delta = (float *)acc_create_unified(NULL, sizeof(float)*(n_hidden + 1));
  output_delta = (float *)acc_create_unified(NULL, sizeof(float)*(n_out + 1));
...
*/
  input_weights = (float (*)[I_SIZE])acc_create_unified(NULL, sizeof(float)*(n_in + 1)*( n_hidden + 1));
  hidden_weights = (float (*)[H_SIZE])acc_create_unified(NULL, sizeof(float)*(n_hidden + 1)*(n_out + 1));

  input_prev_weights = (float (*)[I_SIZE])acc_create_unified(NULL, sizeof(float)*(n_in + 1)*(n_hidden + 1));
  hidden_prev_weights = (float (*)[H_SIZE])acc_create_unified(NULL, sizeof(float)*(n_hidden + 1)*(n_out + 1));
...
void bpnn_free()
{
  ...
  acc_delete_unified((void *) input_units, 0);
  acc_delete_unified((void *) hidden_units, 0);
  free((char *) output_units);

  //free((char *) hidden_delta);
  //free((char *) output_delta);
  acc_delete_unified((void *) hidden_delta, 0);
  acc_delete_unified((void *) output_delta, 0);
  free((char *) target);

  //free((char *) input_weights);
  //free((char *) input_prev_weights);
  acc_delete_unified((void *) input_weights, 0);
  acc_delete_unified((void *) input_prev_weights, 0);

  //free((char *) hidden_weights);
  //free((char *) hidden_prev_weights);
  acc_delete_unified((void *) hidden_weights, 0);
  acc_delete_unified((void *) hidden_prev_weights, 0);
}
