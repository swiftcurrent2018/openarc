...
#endif
  //[DEBUG] Fake computation to access output data, which is needed to measure
  //performance of the unified memory version.
  sum = 0.0F;
  for (j = 1; j <= hid; j++) {
    for (k = 0; k <= in; k++) { 
      sum += input_weights[j][k]; 
    }   
  }
  printf("Sum of input_weights: %lf\n", sum);
  sum = 0.0F;
  for (j = 1; j <= out; j++) {
    for (k = 0; k <= hid; k++) {
      sum += hidden_weights[j][k];
    }
  }
  printf("Sum of hidden_weights: %lf\n", sum);
#ifdef DEBUG0
  end_time0 = gettime();
