...
#endif
	//Fake computation to measure timing of unifiec memory version.
	JSum = 0.0;
	for (i=0; i<rows; i++) {
    	for (j=0; j<cols; j++) {
			JSum += J[i * cols + j];
...
{   
  ...
  FILE *fp;
    ...
    J = (float *)acc_create_unified(NULL, size_I * sizeof(float) );
	c  = (float *)acc_create_unified(NULL, sizeof(float)* size_I) ;
...
*/
	dN = (float *)acc_create_unified(NULL, sizeof(float)* size_I) ;
    dS = (float *)acc_create_unified(NULL, sizeof(float)* size_I) ;
    dW = (float *)acc_create_unified(NULL, sizeof(float)* size_I) ;
    dE = (float *)acc_create_unified(NULL, sizeof(float)* size_I) ;    
...
#if VERIFICATION == 1
{
	...
	free(I);
	//free(J);
	acc_delete_unified(J, 0);
	free(iN); free(iS); free(jW); free(jE);
    //free(dN); free(dS); free(dW); free(dE);
    acc_delete_unified(dN, 0); acc_delete_unified(dS, 0); acc_delete_unified(dW, 0); acc_delete_unified(dE, 0);

	//free(c);
	acc_delete_unified(c, 0);
