...
inputCFiles=( "lud.c" "lud_omp.c" "common.c" )
inputHeaderFiles=( "common.h" )
inputFiles=( "${inputCFiles[@]}" "${inputHeaderFiles[@]}" )

# OpenARC options (common to all inputs) to translate the benchmark.
#openarcOptionBase=( "macro=VERIFICATION=1,HOST_MEM_ALIGNMENT=1,ENABLE_OPENACC=1" )
openarcOptionBase=( "macro=VERIFICATION=1,ENABLE_OPENACC=1" )
...
do
	...
	inputOpenARCOption="macro=_M_SIZE_=${inputClass}"
	k=0 
...
do
	cd ${workDir}
	inputClass=${inputData[$i]}
	if [ $inputClass -eq 4096 ]; then
		tInputData="64.dat"
	elif [ $inputClass -eq 65536 ]; then
		tInputData="256.dat"
	elif [ $inputClass -eq 262144 ]; then
		tInputData="512.dat"
	elif [ $inputClass -eq 4194304 ]; then
		tInputData="2048.dat"
	fi
	echo " " >> ${logFile}
	echo "## Input Data : ${tInputData}" >> ${logFile}
	# Input-specific options to run the benchmark.
	inputOption="-i ${rodinia}/data/lud/${tInputData}"
