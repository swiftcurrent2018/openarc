...
#define DESC_LINE "/* NPROCS = %d CLASS = %c */\n"
...
//  }
  ...
  fprintf(fp, "#define NITER_DEFAULT  %d\n", niter);
  fprintf(fp, "#define DT_DEFAULT     %s\n", dt);
  fprintf(fp, "#define WR_DEFAULT     %d\n", wr_interval);
  fprintf(fp, "#define IOTYPE         %d\n", io);
  if (io) {
    switch (io) {
	case FULL:
	    fprintf(fp, "#define FILENM   \"btio.full.out\"\n");
	    break;
	case SIMPLE:
	    fprintf(fp, "#define FILENM   \"btio.simple.out\"\n");
	    break;
	case EPIO:
	    fprintf(fp, "#define FILENM   \"btio.epio.out\"\n");
	    break;
	case FORTRAN:
	    fprintf(fp, "#define FILENM   \"btio.fortran.out\"\n");
	    fprintf(fp, "%sinteger fortran_rec_sz\n", FINDENT);
	    fprintf(fp, "#define FORTRAN_REC_SZ   %d\n", fortran_rec_size);
	    break;
	default:
	    break;
    }
...
//  fprintf(fp, "%sparameter (dt_default = %s)\n", FINDENT, dt_default);
  ...
  fprintf(fp, "#define NNODES_COMPILED    %d\n", nprocs);
  fprintf(fp, "#define NNODES_XDIM        %d\n", xdiv);

  fprintf(fp, "\n/* full problem size */\n");
  fprintf(fp, "#define ISIZ01   %d\n", problem_size);
  fprintf(fp, "#define ISIZ02   %d\n", problem_size);
  fprintf(fp, "#define ISIZ03   %d\n", problem_size);

  fprintf(fp, "\n/* sub-domain array size */\n");
  fprintf(fp, "#define ISIZ1    %d\n", isiz1);
  fprintf(fp, "#define ISIZ2    %d\n", isiz2);
  fprintf(fp, "#define ISIZ3    ISIZ03\n");
...
//	  FINDENT, ndim1, ndim2, ndim3);
  ...
  fprintf(fp, "#define NPROCS_COMPILED    %d\n", nprocs);
...
void write_ft_info(FILE *fp, int nprocs, char class) 
{
  ...
  fprintf(fp, "#define NTOTAL_F       (1.0*NX*NY*NZ)\n");
}
...
#define LL 400
#include <stdio.h>
#define DEFFILE "npb_config_make.def"
