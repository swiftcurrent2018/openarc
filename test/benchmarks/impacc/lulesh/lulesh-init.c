...
void Domain(Int_t numRanks, Index_t colLoc,
...
#ifdef OPENARC_ARCH
#if OPENARC_ARCH == 0
  m_devtype = acc_device_nvidia;
#elif OPENARC_ARCH == 1
  m_devtype = acc_device_radeon;
#else
  m_devtype = acc_device_xeonphi;
#endif
#endif
  m_numDevs = acc_get_num_devices(m_devtype);
  ...
  if(m_numDevs > 1) {
    ...
    if(numRanks > 1) {
        acc_set_device_num(myRank % m_numDevs, m_devtype);
    }
  }
#endif
#endif
