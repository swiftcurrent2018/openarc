                               ...
                               Int_t numRanks)
{
  ...
  Real_t grindTime1 = ((elapsed_time*1e6)/m_cycle)/(nx*nx*nx);
  Real_t grindTime2 = ((elapsed_time*1e6)/m_cycle)/(nx*nx*nx*numRanks);
  ...
  for (j=0; j<nx; ++j) {
    for (k=j+1; k<nx; ++k) {
      Real_t AbsDiff = FABS(m_e[j*nx+k]-m_e[k*nx+j]);
      TotalAbsDiff  += AbsDiff;

      if (MaxAbsDiff <AbsDiff) MaxAbsDiff = AbsDiff;

      Real_t RelDiff = AbsDiff / m_e[k*nx+j];
